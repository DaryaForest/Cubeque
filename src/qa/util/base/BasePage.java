...
public abstract class BasePage extends DriverContainer {
    ...
    protected void click2(String message, Locator locator, Object... args) {
        Reporter.log(message);
        WebElement element = driver().findElement(locator.getLocator2(args));
        driver().switchTo().defaultContent();
        element.click();
    }
    ...
    protected void typeJS(String message, String value, Locator locator) {
        Reporter.log(message);
        //((JavascriptExecutor) driver()).executeScript("document.getElementById(\"creditCardVerificationNumber\").value = 621;");
        ((JavascriptExecutor) driver()).executeScript("document.getElementById(\""+locator.getValue()+"\").value = \"" + value + "\";");
    }

    protected void selectDropDownListOptionByIndexJS(String message, int index, Locator locator) {
        Reporter.log(message);

        ((JavascriptExecutor) driver()).executeScript("document.getElementById(\""+locator.getValue()+"\").selectedIndex = " + index + ";");
    }
    ...
    protected void waitToBeClickable2(String message, Locator locator, Object... args) {
        waitToBeClickable2(ELEMENT_TIMEOUT_SECONDS, message, locator);
    }
    ...
    protected void waitToBeClickable2(int timeout, String message, Locator locator) {
        Reporter.log(message);

        driver().manage().timeouts().implicitlyWait(timeout, TimeUnit.SECONDS);
        WebDriverWait wait = new WebDriverWait(driver(), timeout);
        wait.until(ExpectedConditions.elementToBeClickable(locator.getLocator2()));
        driver().manage().timeouts().implicitlyWait(ELEMENT_TIMEOUT_SECONDS, TimeUnit.SECONDS);

        // wait until the element on the same place
        WebElement element = driver().findElement(locator.getLocator2());
        Point location;
        do {
            location = element.getLocation();
            BaseActions.wait(0, 500);
    ...
    */
    ...
    protected void scrollToElement2(String logMessage, Locator locator) {
        WebElement element = driver().findElement(locator.getLocator2());
        ((JavascriptExecutor) driver()).executeScript("arguments[0].scrollIntoView();", element);
    }
    ...
    protected void clickRadioButton(String message, Locator locator) throws AWTException {
        Reporter.log(message);
        waitForVisibility("",locator);
        click("Click the 'Place Order' button", locator);
    }
